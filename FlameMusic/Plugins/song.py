...
from FlameMusic.FlameMusicUtilities.helpers.gets import get_url
from FlameMusic.FlameMusicUtilities.helpers.inline import search_markup
...
async def mpthree(_, message: Message):
    ...
    if message.sender_chat:
        return await message.reply_text(
            """
ʏᴏᴜ ᴀʀᴇ ᴀɴ ᴀɴᴏɴʏᴍᴏᴜs ᴀᴅᴍɪɴ!
ʀᴇᴠᴇʀᴛ ᴛᴏ ᴜsᴇʀ ᴀᴄᴄᴏᴜɴᴛ ғʀᴏᴍ ᴀᴅᴍɪɴ ʀɪɢʜᴛs.
"""
        )
    ...
    if url:
        ...
        m = await message.reply_text(
            f"""
<b>🏷ᴛɪᴛʟᴇ:</b> [{title[:25]}]({url})
<b>💡</b> [More Information](https://t.me/{BOT_USERNAME}?start=info_{id})
<b>⚡ sᴜᴘᴘᴏʀᴛᴇᴅ</b> [{BOT_NAME}](t.me/{BOT_USERNAME})
""",
...
/song ᴏʀ /vsong [sᴏɴɢ ᴛɪᴛʟᴇ ᴏʀ ʏᴏᴜᴛᴜʙᴇ ʟɪɴᴋ] - ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ sᴏɴɢs ᴀɴᴅ ᴠɪᴅᴇᴏs
"""
            )
        query = message.text.split(None, 1)[1]
        ...
        try:
            ...
            return await mystic.edit_text(
                f"sᴏɴɢ ɴᴏᴛ ғᴏᴜɴᴅ.\\in**ᴘᴏssɪʙʟᴇ ʀᴇᴀsᴏɴs:** {e}"
            )
        await mystic.delete()
...
async def startyuplay(_, CallbackQuery):
    ...
    m = await CallbackQuery.message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(buttons),
        caption=f"""
<b>🏷️TiTlE:</b> [{title[:25]}]({url})
├ 💡 [More Information](https://t.me/{BOT_USERNAME}?start=info_{id})
└ ⚡ **sᴜᴘᴘᴏʀᴛᴇᴅ:** [{BOT_NAME}](t.me/{BOT_USERNAME})
""",
    )
    os.remove(thumb)
    await CallbackQuery.message.delete()
