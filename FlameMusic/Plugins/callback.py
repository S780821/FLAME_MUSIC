...
Personal Playlist Playing."""
    ...
    if str(smex) == "group":
        ...
        if not _playlist:
            return await CallbackQuery.answer(f"Your Group Has No Playlist On Servers. Try Adding FlameMusics In Playlist.", show_alert=True)
        else:
            await CallbackQuery.message.delete()
            logger_text=f"""Starting Playlist
...
async def cbgroupdel(_,CallbackQuery):  
    ...
    _playlist = await get_note_names(CallbackQuery.message.chat.id)                                    
    if not _playlist:
        return await CallbackQuery.message.reply_text("Group has no Playlist on FlameMusic's Server")
    else:
        titlex = []
        for note in _playlist:
