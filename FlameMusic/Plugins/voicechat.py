...
from FlameMusic.config import get_queue
from pyrogram import Client, filters
from pyrogram.types import Message

from FlameMusic import SUDOERS, app, db_mem, userbot
from FlameMusic.FlameMusicUtilities.database import get_active_chats, is_active_chat
from FlameMusic.FlameMusicUtilities.helpers.checker import checker, checkerCB
...
async def basffy(_, message):
    ...
    try:
        ...
        await message.reply_text(f"ғᴀɪʟ\n**ᴘᴏssɪʙʟᴇ ʀᴇᴀsᴏɴs ᴄᴀɴ ʙᴇ**:{e}")
        return
    await message.reply_text("ᴊᴏɪɴ.")
...
async def baaaf(_, message):
    if len(message.command) != 2:
        await message.reply_text(
            "**ᴜsᴀɢᴇ:**\n/ʟᴇᴀᴠᴇʙᴏᴛ [ᴄʜᴀᴛ ᴜsᴇʀɴᴀᴍᴇ ᴏʀ ᴄʜᴀᴛ ɪᴅ"
        )
        return
    chat = message.text.split(None, 2)[1]
    try:
        ...
        print(e)
        return
    await message.reply_text("ᴛʜᴇ ʙᴏᴛ ʜᴀs sᴜᴄᴄᴇssғᴜʟʟʏ ʟᴇғᴛ ᴛʜᴇ ᴄʜᴀᴛ")
...
async def baujaf(_, message):
    if len(message.command) != 2:
        await message.reply_text(
            "**ᴜsᴀɢᴇ:**\n/ʟᴇᴀᴠᴇ [ᴄʜᴀᴛ ᴜsᴇʀɴᴀᴍᴇ ᴏʀ ᴄʜᴀᴛ ɪᴅ"
        )
        return
    chat = message.text.split(None, 2)[1]
    try:
