...
async def theme_func(_, message):
    ...
    for x in served_chats:
        try:
            await app.send_message(
                x,
                "FlameMusic has just restarted herself. Sorry for the issues.\n\nStart playing after 10-15 seconds again.",
            )
        except Exception:
            pass
    served_chatss = []
    ...
    x = await message.reply_text(f"__Restarting FlameMusic!__")
    await start_restart_stage(x.chat.id, x.message_id)
    os.execvp(
        f"python{str(pyver.split(' ')[0])[:3]}",
        [f"python{str(pyver.split(' ')[0])[:3]}", "-m", "FlameMusic"],
    )
