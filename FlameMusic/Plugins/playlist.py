...
from FlameMusic.FlameMusicUtilities.database.playlist import (
    get_playlist_count,
    _get_playlists,
    get_note_names,
    get_playlist,
...
async def pause_cmd(_, message):
    ...
    if not _playlist:
        await message.reply_text("You have no Playlist on FlameMusic's Server")
    else:
        titlex = []
        j = 0
        count = int(name)
...
async def delgroupplaylist(_, message):
    ...
    if not _playlist:
        await message.reply_text("Group has no Playlist on FlameMusic's Server")
    else:
        titlex = []
        j = 0
        count = int(name)
